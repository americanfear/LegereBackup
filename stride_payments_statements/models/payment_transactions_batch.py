...
class PaymentTransactionsBatch(models.Model):
    ...
    def action_create_bank_statement(self):
        ...
        for record in self:
            ...
            try:
                account_bank_statement_id.button_post()
            except Exception as e:
                _logger.exception("Failed to auto post bank statement. %s", str(e))
                account_bank_statement_id.message_post(
                    body=_('Failed to auto post bank statement. %s', str(e))
                )
    ...
    def action_view_bank_statement(self):
        ...
        action['views'] = [[self.env.ref('stride_accounting.view_bank_statement_form').id, 'form']]
        return action