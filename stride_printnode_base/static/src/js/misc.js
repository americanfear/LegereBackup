...
class DirectPrintMainComponent extends owl.Component {
    ...
    */
    ...
    async willStart() {
        ...
        const storageUserId = browser.localStorage.getItem('stride_printnode_base.user_id');
        ...
        if (!storageUserId || (currentUserId != parseInt(storageUserId))) {
            for (let workstationDevice of WORKSTATION_DEVICES) {
                browser.localStorage.removeItem('stride_printnode_base.' + workstationDevice);
            }
        }

        // Save current user in localStorage
        browser.localStorage.setItem('stride_printnode_base.user_id', currentUserId);

        // Check if devices with IDs from localStorage exists
        const devicesInfo = Object.fromEntries(
            WORKSTATION_DEVICES
                .map(n => [n, browser.localStorage.getItem('stride_printnode_base.' + n)])  // Two elements array
            ...
            for (const workstationDevice in data) {
                if (data[workstationDevice]) {
                    ...
                    let workstationDeviceId = browser.localStorage.getItem(
                        'stride_printnode_base.' + workstationDevice);
