...
class KsSaleOrderInherit(models.Model):
    ...
    def _get_product_ids(self, instance, json_data):
        ...
        odoo_product = self.env['ks.woo.product.template'].search([('ks_woo_product_id', '=', product_id)])
        if not odoo_product:
                ...
                for each_record in tax:
                    ...
                    try:
                        ...
                        if woo_tax_response.status_code in [200, 201]:
                            ...
                            if not current_tax_total:
                                current_tax_total = float(each_record.get('shipping_tax_total') or 0)
    ...
    def _get_order_lines(self, order_json_data, instance):
        ...
        for each_record in order_json_data.get('line_items'):
            ...
            if product:
                ...
                record = {}
                if each_record.get('total_tax') != '0.00':
                    record['tax_id'] = [
                        (6, 0, self.get_tax_ids(order_json_data.get('tax_lines'), each_record.get('taxes'), instance))]
                else:
                    record['tax_id'] = [(6, 0, [])]
                line_items_data = {
                    'ks_woo_order_line_id': each_record.get('id'),
        ...
        for each_rec in order_json_data.get('shipping_lines'):
            ...
            record = {}
            if each_rec.get('total_tax') != '0.00':
                record['tax_id'] = [
                    (6, 0, self.get_tax_ids(order_json_data.get('tax_lines'), each_record.get('taxes'), instance))]
            else:
                record['tax_id'] = [(6, 0, [])]
